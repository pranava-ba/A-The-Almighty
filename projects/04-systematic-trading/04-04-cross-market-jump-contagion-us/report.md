...
# Cross-market jump contagion (US/SGX → Nifty) — Report
*Test overnight transmission from US/SGX moves into the next-day Nifty open using a predetermined-regressor design off the time-zone gap; White-robust OLS + full diagnostics, 2SLS endogeneity check, bivariate VAR/Granger, jump detection, and crisis-interaction regressions separating contagion from interdependence (Forbes-Rigobon). · {{ run_id }} · {{ data_version }} · {{ generated }}*
